...
void test_shared_ptr() {
    ...
    for (int k = 0; k < 2; ++k) {
        ...
        std::sort(ptrs.begin(), ptrs.end(), [](auto&& x, auto&& y) {
            return *x < *y;
        });
...
struct NeitherDefaultNorCopyConstructible {
    ...
    NeitherDefaultNorCopyConstructible& operator=(
        const NeitherDefaultNorCopyConstructible&) = delete;

    NeitherDefaultNorCopyConstructible(NeitherDefaultNorCopyConstructible&&) =
        default;
    NeitherDefaultNorCopyConstructible& operator=(
        NeitherDefaultNorCopyConstructible&&) = default;
...
void* operator new(size_t n, VerySpecialType /*unused*/) {
    return std::malloc(n);
}

void operator delete(void* ptr, VerySpecialType /*unused*/) {
    std::free(ptr);
}
